import { getTwitterProfile, TWITTER_WHO_TO_FOLLOW } from '../../data/twitterProfiles';
  ...
  return (
    <div className="w-[198px] flex-shrink-0">
      ...
      <div className="bg-white border border-[#e6ecf0] rounded-[5px] overflow-hidden mb-[10px]">
        ...
        {TWITTER_WHO_TO_FOLLOW.map((user) => {
          const profile = getTwitterProfile(user.handle, 'default');
          return (
            <div key={user.handle} className="px-[12px] py-[8px] flex items-start gap-[8px] border-b border-[#e6ecf0] last:border-b-0 hover:bg-[#f5f8fa] cursor-pointer transition-colors">
              <img
                ...
                alt={user.name}
                className="w-[36px] h-[36px] rounded-[4px] bg-[#e1e8ed] flex-shrink-0 object-cover"
              />
